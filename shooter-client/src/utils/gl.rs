...
impl Buffer {
    ...
    pub fn bind(&mut self) {
        assert!(!self.data.is_bound, "Attempted to rebind an already bound vertex buffer");
        unsafe {
    ...
    pub fn unbind(&mut self) {
        assert!(self.data.is_bound, "Attempted to unbind an already unbound vertex buffer");
        unsafe {
            gl::BindBuffer(self.data.target, 0)
        }
...
impl VertexArray {
    pub fn bind(&mut self) {
        assert!(!self.is_bound, "Attempted to rebind an already bound vertex array");
