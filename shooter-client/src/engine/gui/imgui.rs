...
impl ImGui {
    pub fn init_gui(w: f32, h: f32) -> ImGui {
...
impl Gui {
    ...
    pub fn update_input(&mut self, input: &Input, dt: f32) {
        self.imgui.update_input(input, dt);
    }
    ...
    pub fn draw_object_list<'a, I: Iterator<Item = &'a str>>(&mut self, names: I) {
        self.imgui.new_frame();
        self.imgui.begin("Objects", true);
        for n in names {
            self.imgui.text(&format!("Model {}", n));
        }
        self.imgui.end();
    }

    pub fn render(&mut self, width: f32, height: f32) {
        self.imgui.render(width, height);
    }
}
